import { ownerHTML, ownerReport } from './owner.js';
...
let data, state = initialState(), selected = 'all', view = 'overview', storageReady = true;
...
document.addEventListener('click',async event=>{
  ...
  try {
    if(el.dataset.ownerCompany){selected=el.dataset.ownerCompany;view='results';render();}
    if(el.dataset.action==='owner-report'){download(ownerReport(rows(),state.records),`growth-os-review-${Date.now()}.json`);notice('Review brief downloaded from local records. Live results still need verification.');}
